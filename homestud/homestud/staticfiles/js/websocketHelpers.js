...
function addSentReducedTopMarginDiv(inner) {
	return '<div class = "message message-sent sent-reduced-top-margin">' + inner + '</div>';
}

function addSentReducedBottomMarginDiv(inner) {
	return '<div class = "message message-sent sent-reduced-bottom-margin">' + inner + '</div>';
}
		...
		if ($first_message.hasClass('message-sent')) {
			...
			$('#chat-dialog').prepend(
				addMessageContainerDiv(
					...
					'<div class = "message message-sent message-date-created">' + date_created + '</div>'
				)
			);
		}
		else {
			$('#chat-dialog').prepend(
				addMessageContainerDiv(
					...
					'<div class = "message message-sent message-date-created">' + date_created + 
					'</div>'
				)
			);
		}
...
function addOpponentMessage(
		...
		if ($last_message.hasClass('message-received')) {
			...
			$('#chat-dialog').append(
				addMessageContainerDiv(
					...
					'<div class="receiver-bubble">' + sender.charAt(0).toUpperCase() + 
					'</div>' + 
		...
		// Last message is sent by the sender and not opposition
			...
			$('#chat-dialog').append(
				addMessageContainerDiv('<div class="message-received-container">' + 
					...
					sender.charAt(0).toUpperCase() + '</div>' + 
					addMessageReceivedContainerDiv(message) + '</div>' + 
					addMessageReceivedDateCreatedContainer(date_created)
				)
			);
		}
	...
	// If we're prepending to the front of the chat on message load
		...
		if ($first_message.hasClass('message-received')) {
			...
			$('#chat-dialog').prepend(
				addMessageContainerDiv(
					'<div class="message-received-container">' + '<div class="receiver-bubble">' + 
					sender.charAt(0).toUpperCase() + '</div>' + 
		...
		// First message is sent by the sender and not opposition
			...
			$('#chat-dialog').prepend(
				addMessageContainerDiv('<div class="message-received-container">' + 
				'<div class="receiver-bubble">' + sender.charAt(0).toUpperCase() + 
				'</div>' + addMessageReceivedContainerDiv(message) + '</div>' + 
				addMessageReceivedDateCreatedContainer(date_created)
				)
			);
		}
